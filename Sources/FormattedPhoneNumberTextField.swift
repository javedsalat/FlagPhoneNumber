//
//  FormattedPhoneNumberTextField.swift
//  CollectionViewDemo
//
//  Created by Javed Salat on 05/07/21.
//

import UIKit

open class FormattedPhoneNumberTextField: UITextField {
    ...
    /// The size of the flag button
        ...
        private var leftViewSize: CGSize {
            let width = flagButtonSize.width + getWidth(text: phoneCodeTextField.text!) + 17
            let height = bounds.height
            return CGSize(width: width, height: height)
        }

        private var phoneCodeTextField: UITextField = UITextField()
        private var pickerImage = UIButton()
    ...
    private func setupPickerImageView() {
        ...
        pickerImage.setImage(image, for: .normal)
        pickerImage.imageView?.contentMode = .scaleAspectFit
        pickerImage.tintColor = textColor
        pickerImage.translatesAutoresizingMaskIntoConstraints = false
        pickerImage.addTarget(self, action: #selector(displayCountries), for: .touchUpInside)
    }
        ...
        private func setupLeftView() {
            ...
            leftView?.addSubview(pickerImage)
            let gesture = UITapGestureRecognizer(target: self, action: #selector(displayCountries))
            ...
            NSLayoutConstraint(item: pickerImage, attribute: .leading, relatedBy: .equal, toItem: phoneCodeTextField, attribute: .trailing, multiplier: 1, constant: 5).isActive = true
            NSLayoutConstraint(item: pickerImage, attribute: .centerY, relatedBy: .equal, toItem: leftView, attribute: .centerY, multiplier: 1, constant: 0).isActive = true
            
            NSLayoutConstraint(item: pickerImage, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 0, constant: 15).isActive = true
            NSLayoutConstraint(item: pickerImage, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 0, constant: 20).isActive = true
        ...
        @objc private func displayCountries() {
            ...
            case .list:
                (delegate as? FormattedTextFieldDelegate)?.fpnDisplayCountryList()
            }
        }
        ...
        private func fpnDidSelect(country: FPNCountry) {
            (delegate as? FormattedTextFieldDelegate)?.fpnDidSelectCountry(name: country.name, dialCode: country.phoneCode, code: country.code.rawValue)
        ...
        @objc private func didEditText() {
            if let phoneCode = selectedCountry?.phoneCode, let number = text {
                ...
                if let validPhoneNumber = getValidNumber(phoneNumber: cleanedPhoneNumber) {
                    ...
                    (delegate as? FormattedTextFieldDelegate)?.fpnDidValidatePhoneNumber(textField: self, isValid: true)
                } else {
                    nbPhoneNumber = nil
                    ...
                    (delegate as? FormattedTextFieldDelegate)?.fpnDidValidatePhoneNumber(textField: self, isValid: false)
                }
            }
        }
...
@objc
public protocol FormattedTextFieldDelegate: UITextFieldDelegate {
    func fpnDidSelectCountry(name: String, dialCode: String, code: String)
    func fpnDidValidatePhoneNumber(textField: FormattedPhoneNumberTextField, isValid: Bool)
